...
class InfiniteVocabEmbedding(nn.Module):
    ...
    def initialize_vocab(self, vocab: List[torch.Tensor]):
        ...
        unique_vocab = []
        for tensor in vocab:
            if not any(torch.equal(tensor, existing) for existing in unique_vocab):
                unique_vocab.append(tensor)
        self.vocab = OrderedDict(
            (tensor.clone().detach(), idx) for idx, tensor in enumerate(unique_vocab)
        )
    ...
    def extend_vocab(self, vocab: List[torch.Tensor], exist_ok=False):
        ...
        new_entries = []
        for tensor in vocab:
            if not any(torch.equal(tensor, key) for key in self.vocab.keys()):
                new_entries.append(tensor)
        ...
        for tensor in new_entries:
            self.vocab[tensor.clone().detach()] = len(self.vocab)
    ...
    def tokenizer(self, words: Union[torch.Tensor, List[torch.Tensor]]):
        ...
        if isinstance(words, torch.Tensor):
            for key, idx in self.vocab.items(): #is this too slow
                if torch.equal(words, key):
                    return idx
            raise ValueError("tensor not found in vocab")
        else: #is a list of tensors
            return [self.tokenizer(tensor) for tensor in words]
